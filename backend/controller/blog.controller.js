...
export const createBlog = async (req, res) => {
    try {
        ...
        if (!allowedFormats.includes(blogImage.mimetype)) {
            return res.status(400).json({ message: "Invalid photo format. Only webp, jpg and png are allowed" });
        }
...
export const updateBlog = async (req, res) => {
    const { id } = req.params;
    const { title, about, category, adminName, adminPhoto, createdBy, blogImage } = req.body;
    ...
    try {
        ...
        if (adminPhoto) updateData.adminPhoto = adminPhoto;
        if (createdBy) updateData.createdBy = createdBy;

        if (req.files && req.files.blogImage) {
            const cloudinaryResponse = await cloudinary.uploader.upload(req.files.blogImage.tempFilePath);

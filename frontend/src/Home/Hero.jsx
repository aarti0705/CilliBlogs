...
function Hero() {
  const { blogs } = useAuth();
  console.log("heor_blogs:",blogs);
  return (
    <div className='container mx-auto my-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6'>
      {blogs && blogs.length > 0 ? (
        blogs.slice(0, 4).map((element) => {
          return <Link to={`/blog/${element._id}`} key={element._id} className='bg-blue-300 rounded-lg hover:shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300'>
            ...
            <div className='p-6 flex items-center'>
              <img src={element.adminPhoto} alt='AdminPhoto' className='w-12 h-12 rounded-full border-2 border-yellow-400 '/>
              <div className='ml-4'>

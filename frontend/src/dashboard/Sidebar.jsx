...
function Sidebar({setComponent}) {
    ...
    return (
        <>
            ...
            <div className={`w-64 h-full shadow-lg fixed top-0 left-0 bg-sky-200 transition-transform duration-300 transform sm:translate-x-0 ${show ? "translate-x-0" : "-translate-x-full"} `}>
                    ...
                    <div className='text-center'>
                        <img className='w-24 h-24 rounded-full mx-auto mb-2 ' src={profile?.user?.user?.photo?.url} alt="Profile" />
                        <p className='text-lg font-semibold'>{profile?.user?.name}</p>
                    </div>
                    <ul className='space-y-6 mx-4'>
                        <button onClick={() => handleComponents("My Blog")} className='w-full px-4 py-2 bg-green-500 rounded-lg hover:bg-green-700 transition duration-300'>MY BLOGS</button>
                        ...
                        {/* <button onClick={() => handleComponents("Update Blog")} className='w-full px-4 py-2 bg-yellow-500 rounded-lg hover:bg-yellow-700 transition duration-300'>Update</button> */}
